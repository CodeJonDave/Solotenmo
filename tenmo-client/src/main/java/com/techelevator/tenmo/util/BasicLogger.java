...
public class BasicLogger {
	...
	static {
		try {
			infoPw = getPrintWriter(AppConstants.INFO_LOG_FILENAME);
			warnPw = getPrintWriter(AppConstants.WARN_LOG_FILENAME);
			errorPw = getPrintWriter(AppConstants.ERROR_LOG_FILENAME);
	...
	private static PrintWriter getPrintWriter(String logFileName) throws FileNotFoundException {
		...
		if (!userDir.endsWith(AppConstants.DIRECTORY_NAME)) {
			userDir += File.separator + AppConstants.DIRECTORY_NAME;
		}
